...
if ( ! class_exists('WoobizzBodyOptions') ) {
	class WoobizzBodyOptions{
		...
		public function __construct( 
			...
			$this->_prefix	= $prefix;
			$this->_url		= ( $url ) ? $url : plugins_url().'/woobizz-body';
			$this->_folder	= $folder;
		...
		function create_page() {
			if ( $this->_options ) {
				...
				if ( isset( $tabs ) ) {
					...
					foreach ( $tabs as $tab ) {
						...
						if ( isset( ${'tab_'.$tab['id']} ) ) {
							foreach ( ${'tab_'.$tab['id']} as $tab_content ) {
								$option = new WoobizzBodyOptionsGenerator( $tab_content, $this->_prefix );
								$div_contents .= $option;
							}
						} else {
					...
					foreach ( ${'tab_0'} as $tab_content ) {
						$option = new WoobizzBodyOptionsGenerator( $tab_content );
						$div_contents .= $option;
					}
			...
			$html = '<div id="theme-options-wrapper">';
				...
				//$html.= '<div id="titleplugin"><p>WOOBIZZ BODY 1.0.0</small></p></div>';
...
if ( ! class_exists( 'WoobizzBodyOptionsGenerator' ) ) {
	class WoobizzBodyOptionsGenerator {
...
if ( ! class_exists( 'WhitelabelSidebars' ) ) {
	...
	static $_count = 0;
	public static $_prefix = WOOBIZZBODY;
	public $_url;
	public $_folder;
